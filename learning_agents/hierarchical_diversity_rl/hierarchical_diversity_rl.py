...
class Hierarchical_Diversity_RL:
    """
    The implementation of the meta-controller
    """
    def __init__(self, env, config, logger=None):
        ...
        self.subgoal_k = None  # number of diverse policies for each subgoal
        ...
        self.meta_eps = self.config.meta_eps        # meta exploration factor
        self.gamma = self.config.gamma
        self.lr = self.config.lr
        self.n_episode = config.n_episode
        self.debug_info_freq = 30       # frequency of printing debug info
    ...
    def _init_variables(self):
        ...
        self.subgoal_k = {subgoal: self.agents_low[subgoal].k for subgoal in self.agents_low}
    ...
    def get_skill_q_values(self, meta_state, subgoal):
        ...
        return [self.meta_q_table[(*meta_state, self.subgoal_name_to_subgoal_id[subgoal], i)] for i in range(self.subgoal_k[subgoal])]
    ...
    def select_skill(self, meta_state, subgoal, epsilon=1.0):
        if np.random.uniform() > epsilon:
            ...
            self.subgoal_skill_z[subgoal] = (self.subgoal_skill_z[subgoal] + 1) % self.subgoal_k[subgoal]
    ...
    def _select_next_subgoal(self, candidate_subgoals, next_meta_state):
        """
        ...
        for next_subgoal in candidate_subgoals:
            next_skill_q = [self.meta_q_table[(*next_meta_state, self.subgoal_name_to_subgoal_id[next_subgoal], z_i)] for z_i in range(self.subgoal_k[next_subgoal])]
    ...
    def _meta_update(self, meta_state, subgoal, z, skill_reward, next_meta_state, linearization_done):
        ...
        if linearization_done:
            ...
            next_subgoal = self._select_next_subgoal(list(self.next_subgoals[subgoal]), next_meta_state)
            next_q_values = [self.meta_q_table[(*next_meta_state, self.subgoal_name_to_subgoal_id[next_subgoal], z_i)] for z_i in
                             range(self.subgoal_k[next_subgoal])]
    ...
    def meta_q_update(self, meta_state, subgoal, z, skill_reward, next_meta_state, linearization_done):
        # train with the latest experience
        self._meta_update(meta_state, subgoal, z, skill_reward, next_meta_state, linearization_done)
        # save the experience to meta memory
        self.meta_memory.append((meta_state, subgoal, z, skill_reward, next_meta_state, linearization_done))
    ...
    def post_episode_update(self):
        ...
        # the agent reaches the final goal state if self.curr_linearization_score is equal to the number of subgoals
    ...
    def post_skill_training(self, subgoal_skill_history, subgoal, skill_z, is_skill_success, linearization_score):
        ...
        if self.config.log_linearization_score_details:
            ...
            if self.logger is not None:
                self.logger.log(log_info, prefix='meta_controller')
        ...
        for i_episode in range(self.n_episode + 5):
            ...
            for subgoal_idx, subgoal in enumerate(self.landmark_sequences[self.curr_linearization_idx]):
                ...
                if not curr_skill_success:
                    ...
                    executed_subgoal_skills.extend([subgoal, selected_skill_z])
                    ...
                    print(f'[INFO:Meta] subgoal {subgoal} already satisfied in the linearization {self.landmark_sequences[self.curr_linearization_idx]}.')
                    raise NotImplementedError
                ...
                self.meta_q_update(meta_state=self.curr_meta_state, subgoal=subgoal,
                                   z=self.subgoal_skill_z[subgoal],
                                   skill_reward=self.curr_skill_reward,
                                   next_meta_state=next_meta_state,
                                   linearization_done=linearization_done)
                ...
                self.post_skill_training(subgoal_skill_history=self.curr_subgoal_skill_sequence,
                                         subgoal=subgoal, skill_z=self.subgoal_skill_z[subgoal],
                                         is_skill_success=self.is_skill_success,
                                         linearization_score=self.curr_linearization_score)
            ...
            if (self.config.args.render and (self.i_episode + 1) % self.config.render_freq == 0) or (self.is_skill_success and self.config.args.render_success):
                if self.i_episode >= last_render_episode + self.config.render_freq - 1:
                    ...
                    for rgb_s in low_level_traj:
                        cv2.imshow('diversity q learning', cv2.cvtColor(rgb_s, cv2.COLOR_RGB2BGR))
                        k = cv2.waitKey(50)
                        if k == ord('q'):
                            break
            ...
            if not is_testing:
                ...
                if (i_episode + 1) % self.debug_info_freq == 0:
                    print(f'[INFO:Meta] linearization values: {self.linearization_values}')
                # logging
                if self.logger is not None:
                    log_info = addict.Dict()
                    log_info.total_step = self.total_step
